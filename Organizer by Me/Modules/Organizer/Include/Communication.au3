...
Func MailSlot_Create($mailslotName, $ip=".")
	Local $result = DllCall("kernel32.dll", "ptr", "CreateMailslotW", "wstr", "\\"&$ip&"\mailslot"&"\"&$mailslotName, "dword", 0, "dword", 0, "ptr", 0)
...
Func MailSlot_Write($mailslotName, $data, $ip=".")
	Local $result = DllCall("kernel32.dll", "ptr", "CreateFileW", "wstr", "\\"&$ip&"\mailslot"&"\"&$mailslotName, "dword", 0x40000000, "dword", 1, "ptr", 0, "dword", 3, "dword", 0, "ptr", 0)
...
Func MailSlot_Params($data)
	If $data = "" Then
		Local $empty[1] = [0]
		Return $empty
	EndIf
	Local $params = StringSplit($data, @TAB, 1)
	For $i = 1 To $params[0]
		Local $split = StringSplit($params[$i], "\\", 1)
		For $j = 1 To $split[0]
			$split[$j] = StringReplace($split[$j], "\t", @TAB)
		Next
		$params[$i] = $split[1]
		For $j = 2 To $split[0]
			$params[$i] = $params[$i] & "\\" & $split[$j]
		Next
	Next
	Return $params
EndFunc
