...
; Fonction d'encodage Texte>Base64
Func Base64_encode($decoded, $urlMode=True)
	Local $object = ObjCreate("Microsoft.XMLDOM")
	If Not IsObj($object) Then
		Return SetError(1, 0, "")
	EndIf
	Local $element = $object.createElement("Tmp")
	$element.DataType = "bin.base64"
	$element.NodeTypedValue = Binary($decoded)
	Local $resultat = $urlMode _
		? StringReplace(StringReplace(StringReplace($element.Text, "+", "-"),"/", "_"), @LF, "") _
		: $element.Text
	Return $resultat
EndFunc

; Fonction de décodage Base64>Texte
Func Base64_decode($encoded, $urlMode=True)
	Local $object = ObjCreate("Microsoft.XMLDOM")
	If Not IsObj($object) Then
		Return SetError(1, 0, "")
	EndIf
	Local $element = $object.createElement("Tmp")
	$element.DataType = "bin.base64"
	$element.Text = $urlMode _
		? StringReplace(StringReplace($encoded, "-", "+"), "_", "/") _
		: $encoded
	Local $resultat = $element.NodeTypedValue
	Return $resultat
EndFunc

; Fonction de pseudo-hash Texte>Base64
Func Base64_hash($source, $urlMode=True)
	Local $hash = Base64_encode($source, $urlMode)
	If @error Then
		Return SetError(1, 0, "")
	EndIf
	$hash = StringReplace($hash, "-", "")
	$hash = StringReplace($hash, "+", "")
	$hash = StringReplace($hash, "_", "")
	$hash = StringReplace($hash, "/", "")
	$hash = StringReplace($hash, "=", "")
	Return $hash
EndFunc

; Fonction de récupération des propriétés étendues du programme (script ou exécutable)
